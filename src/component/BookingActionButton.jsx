...
const BookingActionButton = ({
  ...
  const onSubmit = async (e) => {
    ...
    try {
      // 1. Better-Auth populates the JWT into a cookie named 'better-auth.session_token'
      // or directly inside the active session request payload headers.
      const sessionContext = await authClient.getSession();

      // 2. Safely extract the token string generated by the JWT plugin
      // const token =
      //   sessionContext?.data?.session?.token || sessionContext?.data?.token;
      const { data: tokenData } = await authClient.token();

      // console.log("Extracted JWT Token:", tokenData?.token); // Keep this to verify the token string is loading

      // 3. Make the fetch call to your Express API, uncommenting the Authorization header
      fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/bookings`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${tokenData?.token || ""}`, // Sends the actual token string to your backend
        },
