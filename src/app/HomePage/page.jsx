    ...
    <div className="bg-slate-50 min-h-screen text-slate-800 font-sans">
      ...
      <section className="max-w-7xl mx-auto px-4 py-16 sm:py-24">
        ...
        {loading ? (
          ...
          <div>
            {/* 1. The checkbox MUST have the 'peer' class and be a direct sibling to the grid and button */}
            <input
              type="checkbox"
              id="view-more-toggle"
            ...
            {/* 2. The Grid Container - watches the peer state */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              ${index >= 6 ? "hidden peer-checked:flex" : "flex"}`}
                >
                  {/* Car Image container */}
            ...
            {/* 3. Action Button Label - target rules are now attached directly to the label container via peer styles */}
            {cars.length > 6 && (
              <div className="flex justify-center mt-12">
                <label
                  htmlFor="view-more-toggle"
                  className="inline-flex items-center justify-center px-6 py-3 text-sm font-semibold text-blue-600 border-2 border-blue-600 hover:bg-blue-50 bg-white rounded-xl transition shadow-sm active:scale-95 cursor-pointer select-none
              peer-checked:text-slate-600 peer-checked:border-slate-300 peer-checked:hover:bg-slate-50"
                >
                  {/* Using basic CSS logic inside the template: if it's checked, display shifts */}
                  <span className="block peer-checked:hidden">
                    <Link href="/ExploreCarsPage">View More</Link>
                  </span>
